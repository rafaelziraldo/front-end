...
import { setActive, startEliminarTipoIngreso, startTipoIngreso } from '../../actions/tipoIngreso';
...
export const TablePartial=()=> {
  ...
  const { tipoIngresoList } = useSelector( state => state.tipoIngreso);
  ...
  const handleButtonEdit=(e)=>{
    ...
    let id=e.target.id
    let tipoIngreso = tipoIngresoList.find( tp => tp._id ===id );
   
    dispatch(setActive(tipoIngreso))
    dispatch(verEdit())
  }
  const handleButtonBorrar=(e)=>{
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let id=e.target.id
        dispatch(startEliminarTipoIngreso(id))
